...
ws() {
    ...
    monitor_workspaces=$(hyprctl monitors -j | jq -r '[.[] | .activeWorkspace.id]')
    ...
    for ((i=1; i<=workspaces; i++)); do
        ...
        elif echo "$monitor_workspaces" | jq -e ".[] | select(. == $i)" >/dev/null; then
            workspace_class+=("active-other")
...
stdbuf -oL socat -U - UNIX-CONNECT:"$SOCKET" | while read -r line; do
    if [[ "$line" == workspace* ]] || [[ "$line" == createworkspace* ]] || [[ "$line" == destroyworkspace* ]] || [[ "$line" == focusedmon* ]]; then
        ws
    fi
done
