...
# 43PR/dotfiles installer changed for corven-dotfiles
...
#   2. Verifies that the system has II (Illogical-impulse) Installed
#   3. Detects the available package manager(s)
#   4. Splits packages.txt into "official repo" vs "AUR-only" and
#      installs each with the right tool, so a single AUR-only or
#      unresolvable name never aborts the whole install
#   5. Backs up existing ~/.config
#   6. Installs this repository's configuration
#
...
if [[ ! -d "$CONFIG_DIR/illogical-impulse" ]]; then
    error "Illogical-Impulse is not installed.
    Refer to https://github.com/end-4/dots-hyprland.
    Or launch the install-illogical-impulse.sh script in $REPO_DIR."
    exit 1
fi
...
printf '\033[1;32m       Corven Hyprland Setup Ready       \033[0m\n'
printf '\033[1;32m========================================\033[0m\n'
printf '\n'
